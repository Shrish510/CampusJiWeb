document.addEventListener('DOMContentLoaded', function () {
    ...
    function filterContacts(searchTerm) {
        ...
        contactRows.forEach(row => {
            ...
            const profession = row.querySelector('.contact-profession').textContent.toLowerCase();
            ...
            if (isVisible) {
                ...
                else if (category === 'campus-services') visibleCategories.campusServices++;
                else if (category === 'local-services') visibleCategories.localServices++;
            } else {
        ...
        if (authoritiesHeader) authoritiesHeader.style.display = visibleCategories.authorities > 0 ? '' : 'none';
        if (campusServicesHeader) campusServicesHeader.style.display = visibleCategories.campusServices > 0 ? '' : 'none';
        if (localServicesHeader) localServicesHeader.style.display = visibleCategories.localServices > 0 ? '' : 'none';
        ...
        if (visibleCount === 0) {
            ...
            if (searchTerm === '') {
                if(resultsCounter) resultsCounter.textContent = `Showing all ${contactRows.length} contacts`;
            } else {
                if(resultsCounter) resultsCounter.textContent = `Found ${visibleCount} contact${visibleCount !== 1 ? 's' : ''} matching your search`;
            }
        }
    }
